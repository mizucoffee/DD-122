...
//        this.mOnListMenuClickListener = onListMenuItemClickListener;
    ...
    public void onBindViewHolder(final ViewHolder vh, int position) {
        ...
        Picasso.with(context).load("http://" + context.getServerConnection().getAddress() + "/api/recorded/" + program.getId() + "/preview.png").into(vh.imageView);
        //TODO: IP変更
        vh.playBtn.setOnClickListener((v) -> {
            Uri uri = Uri.parse("http://" + context.getServerConnection().getAddress() + "/api/recorded/" + program.getId() + "/watch.mp4");
